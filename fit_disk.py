...
def fit_disk(ncomp = 10):
    ...
    plt.figure(1)
    plt.clf()
    im = plt.scatter(ra, dec, c=np.log10(nstar), s=2.0, 
    ...
    plt.figure(2)
    plt.clf()
    im = plt.scatter(ra, dec, c=np.log10(nbest), s=2.0, 
                     linewidth=0, cmap='gist_ncar', alpha=0.5,
                     vmin = 0.7, vmax=np.log10(maxn))
    color_bar = plt.colorbar(im)
    color_bar.set_alpha(1)
    ...
    plt.figure(3)
    plt.clf()
    im = plt.scatter(ra, dec, c=((nbest - nstar) / nstar), s=2.0, 
    ...
    im = plt.scatter(r, ((nbest - nstar) / nstar), c=(np.log10(nstar)), s=2.0, 
                     linewidth=0, cmap='gist_ncar', alpha=0.5, 
                     vmin = 0.7, vmax = 3)
    color_bar = plt.colorbar(im)
    color_bar.set_alpha(1)
...
def run_emcee(ra, dec, nstars, param=[0.5,1.5,0.2], likelihoodfunction='',
              nwalkers=100, nsteps=50, nburn=400, nthreads=0, pool=None,
              ncomp=10):
    ...
    p0 = [np.concatenate([np.array([ra0, dec0]), 
                          param[2:]*(1. + np.random.normal(0, 0.05, ndim-2))])
          for i in xrange(nwalkers)]
    print len(p0)
